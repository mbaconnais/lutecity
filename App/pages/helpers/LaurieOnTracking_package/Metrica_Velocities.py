...
def calc_player_velocities(team, smoothing=True, filter_='Savitzky-Golay', window=7, polyorder=1, maxspeed = 12):
    ...
    first_half_idx = team[team['Period']==1].index.to_list()
    ...
    for player in player_ids: # cycle through players individually
        ...
        if smoothing:
            if filter_=='Savitzky-Golay':
                ...
                vx.loc[first_half_idx] = signal.savgol_filter(vx.loc[first_half_idx],window_length=window,polyorder=polyorder)
                vy.loc[first_half_idx] = signal.savgol_filter(vy.loc[first_half_idx],window_length=window,polyorder=polyorder)        
                ...
                # calculate first half velocity
                vx.loc[first_half_idx] = np.convolve( vx.loc[first_half_idx] , ma_window, mode='same' ) 
                vy.loc[first_half_idx] = np.convolve( vy.loc[first_half_idx] , ma_window, mode='same' )      
                # calculate second half velocity
